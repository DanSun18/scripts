...
"taskset -c ${LAUNCH_SERVER_SCRIPT_CORE} ${SCRIPT_HOME}/characterization_scripts/xapian/run_server.sh \
	${SERVER_THREADS} ${MAXREQS} ${WARMUPREQS}" &	
...
taskset -c ${LAUNCH_CLIENT_SCRIPT_CORE} ${SCRIPT_HOME}/characterization_scripts/xapian/run_client.sh \
	${QPS} ${CLIENT_THREADS} ${CLIENT_CORES} ${SERVER_MACHINE}
